...
import {faCommentAlt, faCheckSquare} from "@fortawesome/free-solid-svg-icons";
            ...
            </Grid>
            {
              data && data.map(item => (
                <>
                  ...
                  <TablePatients data={item.patients} classes={classes} filtering={filtering}  goPatientsDetails={ goPatientsDetails}i/>
                </>
              ))
            }
          </Grid>
...
const TablePatients = (props) => {
  const { data, classes, filtering, goPatientsDetails } = props;
            ...
            data && data.filter(filtering()).map((row) => (
              <TableRow key={row.id} hover>
                <TableCell align="center">
                  <Avatar className={classes.cardMedia} src={row.patientImage}/>
                </TableCell>
                ...
                <TableCell align="center">{row.time}</TableCell>
                {
                  row.stateDoctor === 1
                  ?
                  <TableCell onClick={() => goPatientsDetails(row)} align="center">
                    <Tooltip placement="bottom" TransitionComponent={Zoom}
                      title='Contestar'>
                      <IconButton  >
                        <FontAwesomeIcon icon={faCommentAlt} color={'#2a3150'}
